...
const PUT_FAVORITE_LIST_URL = (boardNumber: number | string) =>
...
export const putFavoriteRequest = async (
    ...
    const result = await axios
        .put(PUT_FAVORITE_LIST_URL(boardNumber), {}, authorization(accessToken))
        .then((response) => {
...
export const increaseViewCountResquest = async (
    boardNumber: number | string
) => {
    const result = await axios
...
export const getTop3BoardListResquest = async () => {
    const result = await axios
